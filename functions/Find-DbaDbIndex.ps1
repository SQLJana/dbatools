...
.EXAMPLE 
    ...
    [string[]] $sqlInstance = @('MySQLHost1\Instance1', 'MySQLHost2', 'MySQLHost3\Instance1')
...
.NOTES
    ...
    To convert this from an independent function to a dbatools function, do the following replacements and vice versa
    1) Comment Write-Verbose
        and
        Uncomment Write-Message
    2) Comment Write-Warning
        and
        Uncomment Write-Message
    3) Comment Throw
        and
        Uncomment Stop-Function
...
{ 
    ...
    try
    {
        ...
        foreach($instance IN $SqlInstance)
        {
            ...
            $stepName = "Get the list of all databases for [$instance]"
            #--------------------------------------------
            #Write-Message -Level Verbose -Message $stepName
            Write-Verbose $stepName  
            ...
            if ($databases.Count -le 0) 
            {
                $message = "No databases to qualified in [$instance]."
    ...
    catch
    {
        ...
        $message = "Unable to search and find qualifying indexes. Error in step: `"{0}`" `n{1}" -f `
                        $stepName, $ex.Message

        Throw $message
        #Stop-Function -Message $message -Category ConnectionError -ErrorRecord $_ -Target $null -Continue
    }
    finally
    {
        #cleanup + return value if any!        
    }
}
